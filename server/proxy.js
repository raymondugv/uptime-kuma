...
class Proxy {
  ...
  static SUPPORTED_PROXY_PROTOCOLS =
      [ "http", "https", "socks", "socks5", "socks5h", "socks4" ];
  ...
  static async save(proxy, proxyID, userID) {
    ...
    if (!this.SUPPORTED_PROXY_PROTOCOLS.includes(proxy.protocol)) {
      throw new Error(`
          ...
          this.SUPPORTED_PROXY_PROTOCOLS.join(", ")}."`);
    }
  ...
  static createAgents(proxy, options) {
    ...
    case "https":
      httpAgent =
          new HttpProxyAgent({...httpAgentOptions || {}, ...proxyOptions});
...
async function applyProxyEveryMonitor(proxyID, userID) {
  ...
  const monitors = await R.getAll(
      "SELECT id, proxy_id FROM monitor WHERE user_id = ?", [ userID ]);
