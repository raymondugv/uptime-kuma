...
module.exports.maintenanceSocketHandler = (socket) => {
  ...
  socket.on("addMaintenance", async (maintenance, callback) => {
    try {
      ...
      let bean =
          await Maintenance.jsonToBean(R.dispense("maintenance"), maintenance);
  ...
  socket.on("addMonitorMaintenance", async (maintenanceID, monitors,
                                            callback) => {
    try {
      checkLogin(socket);
      ...
      for await (const monitor of monitors) {
        ...
        bean.import({monitor_id : monitor.id, maintenance_id : maintenanceID});
        await R.store(bean);
      }
  ...
  // Add a new monitor_maintenance
  socket.on("addMaintenanceStatusPage", async (maintenanceID, statusPages,
                                               callback) => {
    try {
      checkLogin(socket);
      ...
      for await (const statusPage of statusPages) {
        ...
        bean.import(
            {status_page_id : statusPage.id, maintenance_id : maintenanceID});
        await R.store(bean);
      }
  ...
  socket.on("getMonitorMaintenance", async (maintenanceID, callback) => {
    try {
      ...
      log.debug("maintenance", `Get Monitors for Maintenance: ${
                                   maintenanceID} User ID: ${socket.userID}`);

      let monitors = await R.getAll(
          "SELECT monitor.id, monitor.name FROM monitor_maintenance mm JOIN monitor ON mm.monitor_id = monitor.id WHERE mm.maintenance_id = ? ",
          [
            maintenanceID,
          ]);
  ...
  socket.on("getMaintenanceStatusPage", async (maintenanceID, callback) => {
    try {
      ...
      log.debug("maintenance", `Get Status Pages for Maintenance: ${
                                   maintenanceID} User ID: ${socket.userID}`);
