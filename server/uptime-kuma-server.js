...
/**
 * `module.exports` (alias: `server`) should be inside this class, in order to
...
class UptimeKumaServer {
  ...
  constructor(args) {
    // SSL
    const sslKey = args["ssl-key"] || process.env.UPTIME_KUMA_SSL_KEY ||
                   process.env.SSL_KEY || undefined;
    const sslCert = args["ssl-cert"] || process.env.UPTIME_KUMA_SSL_CERT ||
                    process.env.SSL_CERT || undefined;
    const sslKeyPassphrase = args["ssl-key-passphrase"] ||
                             process.env.UPTIME_KUMA_SSL_KEY_PASSPHRASE ||
                             process.env.SSL_KEY_PASSPHRASE || undefined;
   ...
   *     monitor IDs as keys and monitor objects as values.
   *
  ...
  async getMonitorJSONList(userID) {
    ...
    let monitorList =
        await R.find("monitor", " user_id = ? ORDER BY weight DESC, name", [
          userID,
        ]);
   ...
   * @returns {Promise<Object>} A promise that resolves to an object with
   *     maintenance IDs as keys and maintenances objects as values.
   */
  ...
  async loadMaintenanceList(userID) {
    let maintenanceList =
        await R.findAll("maintenance", " ORDER BY end_date DESC, title", [
   ...
   * @param {boolean} outputToConsole Should the error also be output to
   *     console?
   */
  static errorLog(error, outputToConsole = true) {
    const errorLogStream =
        fs.createWriteStream(Database.dataDir + "/error.log", {flags : "a"});
  ...
  loadPlugins() { this.pluginsManager = new PluginsManager(this); }

  /**
   *
   * @returns {PluginsManager}
   */
  getPluginManager() { return this.pluginsManager; }
