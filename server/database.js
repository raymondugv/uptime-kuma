...
class Database {
  ...
  static async connect(testMode = false, autoloadModels = true, noLog = false) {
    ...
    if (!noLog) {
      ...
      log.info("db",
               "SQLite Version: " + await R.getCell("SELECT sqlite_version()"));
    }
  }
  ...
  static async migrateNewStatusPage() {
    ...
    if (title) {
      ...
      let statusPageCheck =
          await R.findOne("status_page", " slug = 'default' ");
  ...
  static async importSQLFile(filename) {
    ...
    lines = lines.filter((line) => { return !line.startsWith("--"); });
    ...
    let statements = text.split(";")
                         .map((statement) => { return statement.trim(); })
                         .filter((statement) => { return statement !== ""; });
   ...
   */
  static async close() {
    const listener = (reason, p) => { Database.noReject = false; };
   ...
   * @returns {Promise<void>}
   */
  static async shrink() { await R.exec("VACUUM"); }
}
