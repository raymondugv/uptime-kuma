...
class Monitor extends BeanModel {
   ...
   * @returns {boolean}
   */
  isEnabledExpiryNotification() { return Boolean(this.expiryNotification); }

  /**
   * Parse to boolean
   * @returns {boolean}
   */
  getIgnoreTls() { return Boolean(this.ignoreTls); }

  /**
   * Parse to boolean
   * @returns {boolean}
   */
  isUpsideDown() { return Boolean(this.upsideDown); }

  /**
   * Parse to boolean
   * @returns {boolean}
   */
  getGrpcEnableTls() { return Boolean(this.grpcEnableTls); }
  ...
  start(io) {
    ...
    const beat = async () => {
      ...
      if (!previousBeat || this.type === "push") {
        previousBeat = await R.findOne(
            "heartbeat", " monitor_id = ? ORDER BY time DESC", [ this.id ]);
      }
      ...
      if (!isFirstBeat) {
        bean.duration =
            dayjs(bean.time).diff(dayjs(previousBeat.time), "second");
      } else {
        bean.duration = 0;
      }
      ...
      try {
        if (await Monitor.isUnderMaintenance(this.id)) {
          ...
          if (this.auth_method === "basic") {
            basicAuthHeader = {
              Authorization :
                  "Basic " + this.encodeBase64(this.basic_auth_user,
                                               this.basic_auth_pass),
            };
          }

          const httpsAgentOptions = {
            maxCachedSessions : 0, // Use Custom agent to disable session reuse
          ...
          if (process.env.TIMELOGGER === "1") {
            log.debug("monitor", "Cert Info Query Time: " +
                                     (dayjs().valueOf() - certInfoStartTime) +
                                     "ms");
          }

          if (process.env.UPTIME_KUMA_LOG_RESPONSE_BODY_MONITOR_ID ===
              this.id) {
          ...
          let dnsRes = await dnsResolve(this.hostname, this.dns_resolve_server,
                                        this.port, this.dns_resolve_type);
          bean.ping = dayjs().valueOf() - startTime;

          if (this.dns_resolve_type === "A" ||
            ...
            dnsRes.forEach((record) => {
              dnsMessage += `Hostname: ${record.exchange} - Priority: ${
                  record.priority} | `;
            });
            ...
            dnsMessage += `NS-Name: ${dnsRes.nsname} | Hostmaster: ${
                dnsRes.hostmaster} | Serial: ${dnsRes.serial} | Refresh: ${
                dnsRes.refresh} | Retry: ${dnsRes.retry} | Expire: ${
                dnsRes.expire} | MinTTL: ${dnsRes.minttl}`;
          } else if (this.dns_resolve_type === "SRV") {
            dnsRes.forEach((record) => {
              dnsMessage +=
                  `Name: ${record.name} | Port: ${record.port} | Priority: ${
                      record.priority} | Weight: ${record.weight} | `;
            });
            dnsMessage = dnsMessage.slice(0, -2);
          }
          ...
          // Type: Push
          log.debug("monitor", `[${this.name}] Checking monitor at ${
          ...
          if (previousBeat) {
            ...
            if (previousBeat.status !== (this.isUpsideDown() ? DOWN : UP) ||
              ...
              // No need to insert successful heartbeat for push type, so end
              // here
              retries = 0;
          ...
          let res = await axios.get(steamApiUrl, {
            ...
            httpsAgent : CacheableDnsHttpAgent.getHttpsAgent({
              maxCachedSessions :
                  0, // Use Custom agent to disable session reuse
              // (https://github.com/nodejs/node/issues/3940)
              rejectUnauthorized : !this.getIgnoreTls(),
            }),
          ...
          if (res.data.response && res.data.response.servers &&
          ...
          if (dockerHost._dockerType === "socket") {
            ...
            options.baseURL =
                DockerHost.patchDockerURL(dockerHost._dockerDaemon);
          }
          ...
          bean.msg = await mqttAsync(this.hostname, this.mqttTopic,
          ...
          if (response.code !== 1) {
            bean.status = DOWN;
            bean.msg =
                `Error in send gRPC ${response.code} ${response.errorMessage}`;
          } else {
            if (response.data.toString().includes(this.keyword)) {
              ...
              log.debug(
                  ...
                  `GRPC response [${response.data}] + ", but keyword [${
                      this.keyword}] is not in [" + ${response.data} + "]"`);
              bean.status = DOWN;
              bean.msg = `, but keyword [${this.keyword}] is not in [" + ${
                  responseData} + "]`;
            }
          }
          ...
          await postgresQuery(this.databaseConnectionString,
                              this.databaseQuery);
          ...
          bean.msg = await mysqlQuery(this.databaseConnectionString,
                                      this.databaseQuery);
          bean.status = UP;
          bean.ping = dayjs().valueOf() - startTime;
          ...
          try {
            const resp = await radius(
                this.hostname, this.radiusUsername, this.radiusPassword,
      ...
      let isImportant = Monitor.isImportantBeat(
          isFirstBeat, previousBeat?.status, bean.status);
      ...
      if (isImportant) {
        ...
        if (bean.status === DOWN && this.resendInterval > 0) {
          ...
          if (bean.downCount >= this.resendInterval) {
            ...
            log.debug(
                "monitor",
                `[${this.name}] sendNotification again: Down Count: ${
                    bean.downCount} | Resend Interval: ${this.resendInterval}`);
            await Monitor.sendNotification(isFirstBeat, this, bean);
      ...
      if (bean.status === UP) {
        log.debug("monitor",
                  `Monitor #${this.id} '${this.name}': Successful Response: ${
                      bean.ping} ms | Interval: ${
                      beatInterval} seconds | Type: ${this.type}`);
      } else if (bean.status === PENDING) {
        if (this.retryInterval > 0) {
          beatInterval = this.retryInterval;
        }
        log.warn("monitor",
                 `Monitor #${this.id} '${this.name}': Pending: ${
                     bean.msg} | Max retries: ${this.maxretries} | Retry: ${
                     retries} | Retry Interval: ${
                     beatInterval} seconds | Type: ${this.type}`);
      } else if (bean.status === MAINTENANCE) {
        log.warn("monitor",
                 `Monitor #${this.id} '${
                     this.name}': Under Maintenance | Type: ${this.type}`);
      } else {
        log.warn(
            "monitor",
            `Monitor #${this.id} '${this.name}': Failing: ${
                bean.msg} | Interval: ${beatInterval} seconds | Type: ${
                this.type} | Down Count: ${bean.downCount} | Resend Interval: ${
                this.resendInterval}`);
      }
    ...
    // Delay Push Type
    if (this.type === "push") {
      setTimeout(() => { safeBeat(); }, this.interval * 1000);
    } else {
      safeBeat();
    }
  }
  ...
  async makeAxiosRequest(options, finalCall = false) {
    try {
      ...
      if (!finalCall && typeof e.message === "string" &&
   ...
   */
  getPrometheus() { return this.prometheus; }
  ...
  async updateTlsInfo(checkCertificateResult) {
    let tlsInfoBean =
        await R.findOne("monitor_tls_info", "monitor_id = ?", [ this.id ]);
    ...
    if (tlsInfoBean == null) {
      ...
      try {
        ...
        let isValidObjects = oldCertInfo && oldCertInfo.certInfo &&
  ...
  static async calcUptime(duration, monitorID, forceNoCache = false) {
    ...
    // e.g. If the last beat's duration is bigger that the 24hrs window, it will
    // use the duration between the (beat time - window margin) (THEN case in
    // SQL)
    ...
    if (totalDuration > 0) {
      ...
      let status = parseInt(
          await R.getCell("SELECT `status` FROM heartbeat WHERE monitor_id = ?",
                          [ monitorID ]));
   ...
   * @returns {boolean} True if is an important beat else false
   */
  static isImportantForNotification(isFirstBeat, previousBeatStatus,
                                    currentBeatStatus) {
    // * ? -> ANY STATUS = important [isFirstBeat]
    // UP -> PENDING = not important
    // * UP -> DOWN = important
    ...
    if (!isFirstBeat || bean.status === DOWN) {
      ...
      for (let notification of notificationList) {
        try {
          ...
          // Prevent if the msg is undefined, notifications such as Discord
          // cannot send out.
  ...
  async checkCertExpiryNotifications(tlsInfoObject) {
    if (tlsInfoObject && tlsInfoObject.certInfo &&
        tlsInfoObject.certInfo.daysRemaining) {
      const notificationList = await Monitor.getNotificationList(this);

      if (!notificationList.length > 0) {
        // fail fast. If no notification is set, all the following checks can be
        // skipped.
        log.debug("monitor",
        ...
        for (const targetDays of notifyDays) {
          ...
          while (certInfo) {
            ...
            if (certInfo.daysRemaining > targetDays) {
              log.debug("monitor",
                        `No need to send cert notification for ${
                            certInfo.certType} certificate "${subjectCN}" (${
                            certInfo.daysRemaining} days valid) on ${
                            targetDays} deadline.`);
            } else {
              log.debug("monitor",
                        `call sendCertNotificationByTargetDays for ${
                            targetDays} deadline on certificate ${subjectCN}.`);
              await this.sendCertNotificationByTargetDays(
                  subjectCN, certInfo.certType, certInfo.daysRemaining,
                  targetDays, notificationList);
            }
   ...
   */
  async sendCertNotificationByTargetDays(certCN, certType, daysRemaining,
                                         targetDays, notificationList) {
    ...
    for (let notification of notificationList) {
      try {
        ...
        await Notification.send(
            ...
            `[${this.name}][${this.url}] ${certType} certificate ${
                certCN} will be expired in ${daysRemaining} days`);
        sent = true;
      } catch (e) {
        log.error("monitor",
  ...
  static async isUnderMaintenance(monitorID) {
    ...
    for (const maintenanceID of maintenanceIDList) {
      const maintenance =
          await UptimeKumaServer.getInstance().getMaintenance(maintenanceID);
