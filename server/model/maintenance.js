...
const {parseTimeObject, parseTimeFromTimeObject, log} =
    require("../../src/util");
...
class Maintenance extends BeanModel {
  ...
  async toPublicJSON() {
    ...
    let obj = {
      ...
      timezoneOption : this.timezone, // Mainly for dropdown menu, because there
                                      // is a option "SAME_AS_SERVER"
    ...
    if (this.strategy === "manual") {
      ...
      if (this.beanMeta.job) {
        ...
        if (nextRunDate) {
          ...
          let endDate =
              startDateDayjs.add(this.duration, "second").toISOString();
   ...
   * @returns {Object}
   */
  async toJSON(timezone = null) { return this.toPublicJSON(timezone); }
  ...
  getDayOfWeekList() {
    ...
    return JSON.parse(this.weekdays).sort(function(a, b) { return a - b; });
  }
   ...
   */
  getDayOfMonthList() {
    return JSON.parse(this.days_of_month)
   ...
   */
  calcDuration() {
    let duration = dayjs.utc(this.end_time, "HH:mm")
                       .diff(dayjs.utc(this.start_time, "HH:mm"), "second");
  ...
  async run(throwError = false) {
    if (this.beanMeta.job) {
      log.debug("maintenance",
                "Maintenance is already running, stop it first. id: " +
                    this.id);
      this.stop();
    }
    ...
    if (this.strategy === "manual") {
      ...
      this.beanMeta.job = new Cron(
          this.start_date, {timezone : await this.getTimezone()}, () => {
            log.info("maintenance", "Maintenance id: " + this.id +
      ...
      try {
        ...
        let startEvent = (customDuration = 0) => {
          log.info("maintenance",
                   "Maintenance id: " + this.id + " is under maintenance now");

          this.beanMeta.status = "under-maintenance";
          clearTimeout(this.beanMeta.durationTimeout);

          // Check if duration is still in the window. If not, use the duration
          // from the current time to the end of the window
          let duration;
        ...
        if (runningTimeslot) {
          let duration =
              dayjs(runningTimeslot.endDate).diff(current, "second") * 1000;
          log.debug("maintenance", "Maintenance id: " + this.id +
  ...
  getRunningTimeslot() {
    let start = dayjs(this.beanMeta.job.nextRun(
        dayjs().add(-this.duration, "second").toDate()));
  ...
  async getStatus() {
    ...
    if (this.start_date &&
        dayjs().isBefore(dayjs.tz(this.start_date, await this.getTimezone()))) {
      return "scheduled";
    }

    // Check if the maintenance is ended
    if (this.end_date &&
        dayjs().isAfter(dayjs.tz(this.end_date, await this.getTimezone()))) {
      return "ended";
    }
