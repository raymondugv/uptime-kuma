...
class StatusPage extends BeanModel {
  ...
  static async renderHTML(indexHTML, statusPage) {
    ...
    const escapedJSONObject =
        jsesc(await StatusPage.getStatusPageData(statusPage),
    ...
    // manifest.json
    $("link[rel=manifest]")
        .attr("href", `/api/status-page/${statusPage.slug}/manifest.json`);
  ...
  static async getStatusPageData(statusPage) {
    ...
    let incident = await R.findOne(
        "incident", " pin = 1 AND active = 1 AND status_page_id = ? ", [
          statusPage.id,
        ]);
    ...
    const list = await R.find(
        "group", " public = 1 AND status_page_id = ? ORDER BY weight ",
        [ statusPage.id ]);
  ...
  static async getMaintenanceList(statusPageId) {
    try {
      ...
      for (const maintenanceID of maintenanceIDList) {
        let maintenance =
            UptimeKumaServer.getInstance().getMaintenance(maintenanceID);
